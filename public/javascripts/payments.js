...
//      await handleOrder(order, source);
  ...
  const handleSource = async(source) => {
    ...
    switch(source.status) {
      case 'chargeable':
        console.log('POST source back to server');
        break;
      case 'pending':
        console.log('Other interaction required');
        switch(source.flow) {
          case 'redirect':
            console.log("Redirection to" + source.redirect.url);
